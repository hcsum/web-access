...
在开始联网操作前，先判断这次浏览器任务更适合哪种模式：

- **主力浏览器**：自动继承现有登录态、书签、插件，但连接时可能需要处理远程调试授权弹窗。
- **专用浏览器**：更适合已经预先配置好的长期 agent 环境；平时运行通常不需要处理远程调试授权弹窗，但状态与主力浏览器隔离。

如果用户未明确指定，再进入浏览器模式引导。
引导时不能只让用户做二选一，必须先把两种模式的核心差异、各自好处和代价讲清楚，让用户有足够信息自己判断。

只有在确定使用主力浏览器时，才运行默认检查命令：
...
1. **主力浏览器**
  ...
  - 代价：连接时可能偶尔需要处理远程调试授权弹窗；Agent 操作和用户自己的浏览器操作不隔离
2. **专用浏览器**
  - 好处：更适合已经预先配置好的长期 agent 环境；平时运行通常不需要处理远程调试授权弹窗；Agent 操作和用户自己的浏览器操作完全隔离
  - 代价：第一次需要重新登录常用网站、安装常用插件、同步书签；这些状态会保存在专用 profile 中

推荐的话术风格应接近下面这种信息量，而不是只给编号选项：

```text
1. 主力浏览器
  好处：直接复用你现在的登录态、书签和插件，适合临时任务或需要马上访问已登录网站的情况。
  代价：连接时可能需要处理远程调试授权弹窗，而且 Agent 操作和你自己的浏览器操作不隔离。

2. 专用浏览器
  好处：Agent 操作和你的日常浏览完全隔离，更适合长期、稳定、低打扰地运行自动化任务。
  代价：第一次需要在这个专用 profile 里重新登录常用网站、装插件、同步书签。
```
